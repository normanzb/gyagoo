...
($itemname,$itemjuuryou,$itemgold,$itemkouka) = split(/\//,$in{itemkai});
	if ( $chagold < $itemgold ) {
print <<"HTM";
オイオイ、冗談言ってもらっちゃぁ困るぜ。<br>
金が足らねェよ。ボケてんじゃねぇって。<br><br>
金作って出直してきな。
HTM
&shopform;
	} elsif ( $in{itemkai} eq "" ) {
...
&charadatawrt;
print <<"HTM";
<font size=4 color=#$mojiiro3><b>$itemname</b></font> ($itemkouka) だな。<br>
毎度ありがとよ。<br>
まだ何か買っていくか？<br>
HTM
&kounyuu;
...
print <<"HTM";
そのアイテムはすでに容量いっぱいのようだな。<br>
複数個所持できるアイテムは最高999個までしか持てないんだ。<br>
覚えておくといいぜ。<br>
まだ何か買っていくか？<br>
HTM
&kounyuu;
...
&shopform;
			...
			if ( $itemjuuryou == 53 ) { $itemname = "不思議な翼"; }
			if ( $itemjuuryou == 54 ) { $itemname = "エクスポーション"; }
			if ( $itemjuuryou == 59 ) { $itemname = "メガエリクサー"; }
			if ( $itemjuuryou == 60 ) { $itemname = "ギガエリクサー"; }
			if ( $itemjuuryou == 61 ) { $itemname = "テラエリクサー"; }
			if ( $itemjuuryou == 62 ) { $itemname = "タブレットα"; }
			if ( $itemjuuryou == 63 ) { $itemname = "タブレットβ"; }
			if ( $itemjuuryou == 64 ) { $itemname = "タブレットγ"; }
...
&charadatawrt;
print <<"HTM";
<font size=4 color=#$mojiiro3><b>$itemname</font></b> ($itemkouka) だな。<br>
個数増やしておいたぜ。<br>
確認しな<br>
まだ何か買っていくか？
HTM
...
print <<"HTM";
<br>これは$omosa[$itemjuuryou]装備だから<br>
今のお前の職業じゃ装備できねぇけどな。<br>
買っちまったもんはしょうがないよな。<br>
HTM
				}
print <<"HTM";
まだ何か買っていくか？
...
<tr><td><nobr>部位</nobr></td><td><nobr>$bui[$itembasyo]</nobr></td></tr>
...
print <<"HTM";
よう、<font color=#$chamoji><b>$chaname</font></b> じゃないか。<br>
よく来てくれたな。どんな用事だい？<br><br>
HTM
&shopform;
}
...
<input type=submit value="   アイテム購入   ">
</form>
HTM
	}
...
<input type=hidden value="$chaid" name="usrid">
<input type=hidden value="$chapass" name="usrpass">
<input type=submit value="   アイテム売却   ">
</form>
HTM
	}
...
<input type=hidden value="$chaid" name="usrid">
<input type=hidden value="$chapass" name="usrpass">
<input type=submit value="   アイテム鑑定   ">
</form>
HTM
	}
}
...
print "<option value=\"$itemname/$itemjuuryou/$itemgold/$itemkouka\">$itemgold G:$omosa[$itemjuuryou]/$bui[$itembasyo] ● $itemname （$itemkouka）";
	}
close(ITEM);
print <<"HTM";
